...
def _merge_fragments(blobs, strip):
    ...
    MUST stay byte-for-byte identical to the copy in streamlit_app.py so the
    glyphs the model is TRAINED on match the glyphs it sees at INFERENCE.
    Embossed plates break one glyph into vertically-stacked pieces that share
    an x-range; merge any blobs whose horizontal intervals overlap strongly.
